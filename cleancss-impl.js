...
module.exports = function (dirPath, options) {
  ...
  if (options.each) {
    ...
    files.forEach(function (fileName) {
      ...
      var minifyResult = new CleanCSS(
        getUglifyOptions(newName, uglifyConfiguration)
      ).minify(originalCode[fileName]);

      if (minifyResult.errors.length > 0) {
        console.error(fileName, minifyResult.errors);
        throw new Error("error");
      }

      if (minifyResult.warnings.length > 0) {
        console.warn(fileName, minifyResult.warnings);
      }

      writeFile(newName, minifyResult.styles, state);
    });
  } else {
    ...
    if (isEmpty(options.output)) {
      ...
      writeFile(options.output, minifyResult.styles, state);
    }
  }
};
